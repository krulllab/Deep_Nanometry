...
class NormalStochasticBlock1d(nn.Module):
    """
    ...
    def __init__(self, c_in, c_vars, c_out, kernel=3, transform_p_params=True):
        ...
        if transform_p_params:
            self.conv_in_p = nn.Conv1d(c_in, 2 * c_vars, kernel, padding=pad, padding_mode='replicate')
        self.conv_in_q = nn.Conv1d(c_in, 2 * c_vars, kernel, padding=pad, padding_mode='replicate')
        self.conv_out = nn.Conv1d(c_vars, c_out, kernel, padding=pad, padding_mode='replicate')
    ...
    def forward(self,
        ...
        # Define p(z)
        p_mu, p_lstd = p_params.chunk(2, dim=1)
        p = Normal(p_mu, nn.functional.softplus(p_lstd))

        if q_params is not None:
            # Define q(z)
            q_params = self.conv_in_q(q_params)
            q_mu, q_lstd = q_params.chunk(2, dim=1)
            q = Normal(q_mu, nn.functional.softplus(q_lstd))
